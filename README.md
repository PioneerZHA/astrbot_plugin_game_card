...
注意：`playwright install chromium` 是额外步骤，不会由 `pip install -r requirements.txt` 自动完成。如果不想下载 Playwright Chromium，插件会自动尝试使用系统 Chrome/Edge，也可以在配置中填写 `browser_executable_path`。
...
| `browser_executable_path` | 空 | 可选 Chrome/Edge 浏览器可执行文件路径 |
...
如果服务器不方便安装 Playwright Chromium，可以安装 Chrome/Edge，并在配置里填写 `browser_executable_path`。
